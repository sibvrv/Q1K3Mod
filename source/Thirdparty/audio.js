...
let AUDIO_TAB_SIZE = 4096;
let AUDIO_TAB_MASK = AUDIO_TAB_SIZE - 1;
let AUDIO_TAB = new Float32Array(AUDIO_TAB_SIZE * 4); // 4 oscilators
...
let audio_play = (buffer, volume = 1, loop = 0, pan = 0) => {
...
let audio_get_ctx_buffer = (buf_l, buf_r) => {
...
let audio_generate_sound = (
  ...
  let osc1_offset = osc1_waveform * AUDIO_TAB_SIZE;
  let osc2_offset = osc2_waveform * AUDIO_TAB_SIZE;
  let fx_pan_freq = Math.pow(2, fx_pan_freq_p - 8) / row_len;
  let lfo_freq = Math.pow(2, lfo_freq_p - 8) / row_len;
  let c1 = 0;
  let c2 = 0;
  ...
  let band = 0;
  let high = 0;
  let buf_length = buf_l.length;
  let num_samples = attack + sustain + release - 1;
  ...
  for (let j = num_samples; j >= 0; --j) {
    ...
    let k = j + write_pos;
    // LFO
    let lfor = (AUDIO_TAB[osc_lfo_offset + ((k * lfo_freq * AUDIO_TAB_SIZE) & AUDIO_TAB_MASK)] * lfo_amt) / 512 + 0.5;
    let sample = 0;
    let filter_f = fx_freq;
    let temp_f;
    let envelope = 1;
...
let audio_create_song = (row_len, pattern_len, song_len, tracks) => {
  let num_samples = AUDIO_SAMPLERATE * song_len;
  let mix_buf_l = new Float32Array(num_samples);
  let mix_buf_r = new Float32Array(num_samples);
...
let audio_create_sound = (note, instrument, row_len = 5605) => {
  let delay_shift = (instrument[20 /*fx_delay_time*/] * row_len) >> 1;
  let delay_amount = instrument[21 /*fx_delay_amt*/] / 255;
  let num_samples = instrument[13 /*env_attack*/] + instrument[14 /*env_sustain*/] + instrument[15 /*env_release*/] + delay_shift * 32 * delay_amount;
  let buf_l = new Float32Array(num_samples);
  let buf_r = new Float32Array(num_samples);
  audio_generate_sound(row_len, note, buf_l, buf_r, 0, ...instrument);
  audio_apply_delay(delay_shift, delay_amount, buf_l, buf_r);
  return audio_get_ctx_buffer(buf_l, buf_r);
};

let audio_apply_delay = (shift, amount, buf_l, buf_r) => {
