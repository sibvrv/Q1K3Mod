...
let r_buffer = new Float32Array(R_MAX_VERTS * 8);
let r_num_verts = 0;

// 2 vec3 per light [(x,y,z), [r,g,b], ...]
let r_light_buffer = new Float32Array(R_MAX_LIGHT_V3 * 3);
let r_num_lights = 0;
...
// Camera position
let r_camera = vec3(0, 0, -50);
let r_camera_pitch = 0.2;
let r_camera_yaw = 0;
...
let r_vertex_attrib = (shader_program, attrib_name, count, vertex_size, offset) => {
...
let r_push_block = (x, y, z, sx, sy, sz, texture) => {
  let canvas = r_textures[texture].c;
  let index = r_num_verts;
  let tx = sx / canvas.width;
  let ty = sy / canvas.height;
  let tz = sz / canvas.width;

  // top
  let v0 = vec3(x, y + sy, z);
  let v1 = vec3(x + sx, y + sy, z);
  let v2 = vec3(x, y + sy, z + sz);
  let v3 = vec3(x + sx, y + sy, z + sz);

  // bottom
  let v4 = vec3(x, y, z + sz);
  let v5 = vec3(x + sx, y, z + sz);
  let v6 = vec3(x, y, z);
  let v7 = vec3(x + sx, y, z);
